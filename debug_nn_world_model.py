"""
Debug script for NN World Model PPO training on single GPU.
This script is designed for line-by-line debugging with debugger (e.g., pdb, debugpy).

Usage:
    # Run with pdb
    python debug_nn_world_model.py

    # Run with debugpy (for VSCode debugging)
    python -m debugpy --listen 5678 --wait-for-client debug_nn_world_model.py
...
# Set PYTHONPATH so Ray worker subprocesses can also find the module
existing_pythonpath = os.environ.get("PYTHONPATH", "")
os.environ["PYTHONPATH"] = (
    f"{REPO_PATH}:{existing_pythonpath}"
...
CONFIG_NAME = "nn_world_model_adjust_bottle_ppo_openpi_pi05_debug"
