...
**符号说明**:
- `B`: batch_size (环境数量 `num_envs`)
- `N`: action chunk数量 (`num_action_chunks`, 默认50)
- `A`: action维度 (LIBERO=7, RobotWin=14)
- `chunk`: 每次实际执行的action步数 (默认8)

---

### 1. Policy → WanEnv: `chunk_step(actions)`

#### 输入格式

```python
actions: np.ndarray 或 torch.Tensor
形状: [num_envs, num_action_chunks, action_dim]
     [B,        N,               A]

示例 (RobotWin):
  [64, 50, 14]  # 64个环境, 每个输出50步action, 每步14维
```

#### Action流转处理

```python
# 1. Policy输出 (已反归一化的原始物理空间action)
actions: [64, 50, 14]

# 2. WanEnv接收后只取前chunk步 (默认8步)
#    实际使用: actions[:, :chunk, :] → [64, 8, 14]

# 3. 类型转换 (不做归一化)
actions_tensor = torch.from_numpy(actions).to(device)  # [64, 8, 14]

# 4. 拼接condition_action (自回归生成)
# condition_action: [64, 5, 14] (历史5帧的action)
# 拼接后: [64, 5+8, 14] = [64, 13, 14]
actions_tensor = torch.cat([self.condition_action, actions_tensor], dim=1)

# 5. 输入Wan模型生成视频帧
output = self.pipe(
    input_image=batch_input_image,      # 首帧 [PIL]*64
    input_image4=batch_input_image4,    # 前4帧 [PIL*4]*64
    action=actions_tensor,              # [64, 13, 14]
    num_frames=13,                      # 5条件帧 + 8新帧
    ...
)
```
...
✅ **Policy必须输出原始物理空间action**:
- Policy内部使用`norm_stats.json`进行反归一化
- 输出到环境的是原始关节角度/位置值

---

### 2. WanEnv → Policy: `reset()` 返回值
...
obs: dict
{
    "main_images": torch.Tensor,      # [B, 3, 1, 5, 256, 256]
    "wrist_images": None,             # WanEnv不提供wrist
    "states": torch.Tensor,           # [B, action_dim] (初始action)
    "task_descriptions": List[str]    # len=B, 如 ["click the bell"] * 64
}
...
```python
main_images: [num_envs, C, V, T, H, W]
             [64,        3, 1, 5, 256, 256]
...
维度说明:
  ...
  - C: 颜色通道 (RGB=3)
  - V: 视图数量 (单视角=1)
  - T: 时间帧数 (condition_frame_length=5)
  - H, W: 图像尺寸 (256x256)
  
值域: [-1, 1] (已通过trans_norm归一化)
```
...
```python
# 从npy文件加载
episode_data = np.load("traj0.npy", allow_pickle=True).item()

start_items = episode_data["start_items"]  # 初始帧
target_items = episode_data.get("target_items", [])  # 关键帧(KIR模式)

first_frame = start_items[0]
{
    "image": torch.Tensor,          # [3, 256, 256], 值域[0,1]
    "observation.state": np.array   # [14] 初始关节位置
}

# 归一化到[-1,1]
img_tensor = self.trans_norm(first_frame["image"])  
# trans_norm: mean=0.5, std=0.5

# 重复condition_frame_length次
...
### 3. WanEnv → Policy: `chunk_step()` 返回值

#### 输出格式

```python
return (
    obs_list,           # List[dict], len=1
    chunk_rewards,      # torch.Tensor [B, chunk]
    chunk_terminations, # torch.Tensor [B, chunk] (bool)
    chunk_truncations,  # torch.Tensor [B, chunk] (bool)
    infos_list          # List[dict], len=1
)
```
...
obs_list[0] = {
    "main_images": torch.Tensor,      # [B, 3, 1, 5, 256, 256]
    "wrist_images": None,
    "states": torch.Tensor,           # [B, action_dim]
    "task_descriptions": List[str]
}
# 注意: main_images已更新为最新5帧 (滑动窗口)
```

**chunk_rewards**:
```python
形状: [num_envs, chunk]  # [64, 8]
值域: [0, chunk] (累积奖励)

计算流程:
  1. Wan生成8帧视频: [64, 8, 3, 256, 256]
  2. 奖励模型预测每帧: [64*8, 1] → reshape → [64, 8]
  3. 差分奖励: reward_diffs = reward_coef * chunk_rewards - prev_step_reward
  4. 返回: chunk_rewards_tensors (处理后)
```
...
成功判定:
  ...
  max_reward_in_chunk = chunk_rewards.max(dim=1)[0]  # [64]
  success_estimated = max_reward_in_chunk >= success_threshold
  
  # 只在chunk的最后一步标记
  chunk_terminations[:, -1] = success_estimated
```
...
```python
# ========== Policy端 ==========
from rlinf.models.embodiment.openpi import OpenPIActionModel

policy = OpenPIActionModel(cfg)

# 1. Reset获取初始观测
obs, info = wan_env.reset()
# obs["main_images"]: [64, 3, 1, 5, 256, 256]

# 2. Policy预测action
actions, result = policy.predict_action_batch(obs, mode="train")
# actions: [64, 50, 14]  # 64环境, 50步chunk, 14维action

# 3. 执行chunk_step (WanEnv内部只取前8步)
obs_list, rewards, terminations, truncations, infos = wan_env.chunk_step(actions)
# rewards: [64, 8]  # 每环境8步的奖励

# 4. 提取RL训练数据
logprobs = result["logprobs"]  # [64, 50, action_chunk_size]
values = result["values"]      # [64]

# 5. 计算advantage和returns
advantages, returns = compute_gae(
    rewards=rewards.sum(dim=1),  # [64] 每chunk总奖励
    values=values,
...
actor_loss = compute_ppo_loss(
    logprobs=logprobs[:, :8],  # 只取前8步对应的logprobs
...
| 参数 | Policy配置 | WanEnv配置 | 说明 |
|------|-----------|-----------|------|
| **action_dim** | `actor.model.action_dim: 14` | `env.train.action_dim: 14` | 必须一致 |
| **num_action_chunks** | `actor.model.num_action_chunks: 50` | - | Policy输出chunk数 |
| **chunk** | - | `env.train.chunk: 8` | WanEnv每次执行步数 |
| **condition_frame_length** | - | `env.train.condition_frame_length: 5` | 条件帧数量 |
| **num_frames** | - | `env.train.num_frames: 13` | 总帧数=5+8 |
| **image_size** | - | `env.train.image_size: [256, 256]` | 生成图像尺寸 |

⚠️ **关键约束**:
- Policy的`action_dim`必须与WanEnv的`action_dim`一致
- WanEnv只使用Policy输出的前`chunk`步action (默认8步)
- 图像归一化: Policy接收[-1,1], WanEnv内部维护[-1,1]
...
```
Step 0: Reset
  WanEnv → [B,3,1,5,256,256] → Policy
  Policy → [B,50,A] → WanEnv

Step 1: chunk_step (执行action 0-7)
  WanEnv内部:
    1. 取actions[:, :8, :] → [B, 8, A]
    2. 拼接condition_action → [B, 13, A]
    3. Wan生成8帧视频 → 更新current_obs
    4. 奖励模型预测 → [B, 8]
    5. 更新condition_action (滑动窗口)
  
  返回:
    obs: [B,3,1,5,256,256] (最新5帧)
    rewards: [B, 8]
    terminations: [B, 8] (仅最后一步可能为True)

Step 2: 下次chunk_step (执行action 8-15)
  WanEnv内部:
    - condition_action已更新为step 3-7的action
    - 重复上述流程...
```
...
self.condition_action = torch.zeros(
    self.num_envs,
    self.condition_frame_length,
    self.action_dim,  # ✅ 支持RobotWin 14维
)
```
...
| 环境 | 夹爪表示 | 初始化处理 |
|------|---------|-----------|
| **LIBERO** | `-1`表示打开 | `condition_action[:, :, -1] = -1` |
| **RobotWin** | 绝对位置`[0, 0.04]` | 无需特殊处理 (使用0初始化) |

```python
# 环境类型判断
self.is_libero_env = cfg.get("wm_env_type", "libero") == "libero"
self.is_robotwin_env = cfg.get("wm_env_type", "libero") == "robotwin"
